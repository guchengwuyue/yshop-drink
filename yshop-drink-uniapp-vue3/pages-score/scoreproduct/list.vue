	...
	<view>
		...
		<uv-waterfall v-model="list" :add-time="0" ref="uWaterfall" @changeList="changeList" :left-gap="10"
			...
			<template v-slot:list1>
				<view class="score-product-list__card" v-for="item in list1" :key="item.id" @click="goDetail(item)">
					<image :src="item.image" mode="widthFix" class="score-product-list__image" lazy-load></image>
			...
			<template v-slot:list2>
				<view class="score-product-list__card" v-for="item in list2" :key="item.id" @click="goDetail(item)">
					<image :src="item.image" mode="widthFix" class="score-product-list__image" lazy-load></image>
					<view class="score-product-list__title">
						{{item.title}}
					</view>
...
const goDetail = (item) => {
	uni.navigateTo({
		url: '/pages-score/scoreproduct/detail?id=' + item.id
	})
}
