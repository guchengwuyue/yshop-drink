<template>
	...
	<view class="container score-product-orders">
		...
		<view class="score-product-orders__list d-flex flex-column w-100">
			<view class="score-product-orders-item" v-for="item in orders" :key="item.orderId">
				...
				<list-cell :hover="false" last>
					<view class="w-100 d-flex flex-column">
						<view class="flex ">
							<image :src="item.product.image" mode="aspectFill" class="score-product-orders-item__thumb" lazy-load></image>
							<view>
...
const detail = (id) => {
	uni.navigateTo({
		url: '/pages-score/scoreproduct/orderDetail?id=' + id
	})
}
// 确认收到货
