				...
				<view class="order-detail-section">
					...
					<list-cell :hover="false" padding="30rpx 30rpx">
						<view class="w-100 d-flex flex-column position-relative">
							<view class="w-100 d-flex align-items-center mb-40" v-for="(good, index) in order.cartInfo" :key="good.id || `${good.title}-${index}`">
...
const refund = (order) => {
	uni.navigateTo({
		url: '/pages-order/orders/refund?orderId=' + order.orderId + '&payPrice=' + order.payPrice + '&totalPrice=' + order.totalPrice
	})
}
