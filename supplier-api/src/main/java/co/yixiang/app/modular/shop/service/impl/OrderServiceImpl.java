...
import co.yixiang.app.modular.shop.service.mapper.OrderMapper;
...
public class OrderServiceImpl extends ServiceImpl<StoreOrderMapper, StoreOrder>  implements IOrderService {
    ...
    public String addOrderOfOne(OrderVo orderVo, int userId) {
        ...
        if(storeCarts.isEmpty()){
            throw new BadRequestException("购物车空空如也");
        }
        ...
        //插入主表
        if(baseMapper.insert(storeOrder) == 0) throw new BadRequestException("操作失败");;
    ...
    public String addOrderOfTwo(OrderVo orderVo, int userId) {
        ...
        if(storeGoods == null) throw new BadRequestException("商品不存在");
        ...
        switch (type){
            case 1:
                if(storeOrder.getPayStatus() != 0){
                    throw new BadRequestException("只有未付款订单才能取消");
                }
                order.setDeleted(1);
            ...
            case 2:
                if(storeOrder.getShippingStatus() != 1){
                    throw new BadRequestException("只能操作已经发货订单");
                }
            ...
            case 3:
                if(storeOrder.getShippingStatus() == 1 && storeOrder.getOrderStatus() == 0){
                    throw new BadRequestException("已发货订单才能操作退货");
                }
    ...
    public void payYue(StoreOrder order) {
        ...
        if(result <= 0){
            throw new BadRequestException("减余额失败");
        }
